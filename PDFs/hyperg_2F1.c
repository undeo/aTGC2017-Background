 ...
 */
static int
mhyperg_2F1_series(const double a, const double b, const double c,
                  const double x, 
...
static
int
mhyperg_2F1_conj_series(const double aR, const double aI, const double c,
                       double x,
                       gsl_sf_result * result)
{
  if(c == 0.0) {
...
static
int
mhyperg_2F1_luke(const double a, const double b, const double c,
                const double xin, 
                gsl_sf_result * result)
{
  int stat_iter;
...
static
int
mhyperg_2F1_conj_luke(const double aR, const double aI, const double c,
                     const double xin, 
                     gsl_sf_result * result)
{
  int stat_iter;
...
static
int
mhyperg_2F1_reflect(const double a, const double b, const double c,
                   const double x, gsl_sf_result * result)
{
  ...
  if(d_integer) {
    ...
    status_F1 = Mathelp::mhyperg_2F1_series(  a,   b, 1.0-d, 1.0-x, &F1);
    status_F2 = Mathelp::mhyperg_2F1_series(c-a, c-b, 1.0+d, 1.0-x, &F2);
...
int
mgsl_sf_hyperg_2F1_e(double a, double b, const double c,
                       const double x,
                       gsl_sf_result * result)
{
  const double d = c - a - b;
  ...
  if(a >= 0.0 && b >= 0.0 && c >=0.0 && x >= 0.0 && x < 0.995) {
     ...
     */
    return Mathelp::mhyperg_2F1_series(a, b, c, x, result);
  }

  if(fabs(a) < 30.0 && fabs(b) < 10.0) {
    /* a and b are not too large, so we attempt
     * variations on the series summation.
     */
    if(a_neg_integer) {
      return Mathelp::mhyperg_2F1_series(rinta, b, c, x, result);
    }
    ...
    if(b_neg_integer) {
      return Mathelp::mhyperg_2F1_series(a, rintb, c, x, result);
    }

    if(x < -0.25) {
      return Mathelp::mhyperg_2F1_luke(a, b, c, x, result);
    }
    else if(x < 0.5) {
      return Mathelp::mhyperg_2F1_series(a, b, c, x, result);
    }
      ...
      if(fabs(c) > 10.0) {
        ...
        return Mathelp::mhyperg_2F1_reflect(a, b, c, x, result);
      }
    }
  }
  else {
...
int
mgsl_sf_hyperg_2F1_conj_e(const double aR, const double aI, const double c,
                            const double x,
                            gsl_sf_result * result)
{
  ...
  if(   (ax < 0.25 && fabs(aR) < 20.0 && fabs(aI) < 20.0)
    ...
    return Mathelp::mhyperg_2F1_conj_series(aR, aI, c, x, result);
  }
  else if(fabs(aR) < 10.0 && fabs(aI) < 10.0) {
    if(x < -0.25) {
      return Mathelp::mhyperg_2F1_conj_luke(aR, aI, c, x, result);
    }
    else {
      return Mathelp::mhyperg_2F1_conj_series(aR, aI, c, x, result);
    }
...
int
mgsl_sf_hyperg_2F1_renorm_e(const double a, const double b, const double c,
                              const double x,
                              gsl_sf_result * result
                              )
{
  ...
  if(c_neg_integer) {
    if((a_neg_integer && a > c+0.1) || (b_neg_integer && b > c+0.1)) {
      ...
      if(stat != 0) {
        ...
        gsl_sf_result F;
        int stat_F = Mathelp::mgsl_sf_hyperg_2F1_e(a-c+1, b-c+1, -c+2, x, &F);
    ...
    int stat_F = Mathelp::mgsl_sf_hyperg_2F1_e(a, b, c, x, &F);
...
int
mgsl_sf_hyperg_2F1_conj_renorm_e(const double aR, const double aI, const double c,
                                   const double x,
                                   gsl_sf_result * result
                                   )
{
  ...
  if(c_neg_integer) {
    if(a_neg_integer && aR > c+0.1) {
      ...
      if(stat != 0) {
        ...
        gsl_sf_result F;
        int stat_F = Mathelp::mgsl_sf_hyperg_2F1_conj_e(aR-c+1, aI, -c+2, x, &F);
    ...
    double sgn;
    int stat_g = gsl_sf_lngamma_sgn_e(c, &lng, &sgn);
    int stat_F = Mathelp::mgsl_sf_hyperg_2F1_conj_e(aR, aI, c, x, &F);
    int stat_e = gsl_sf_exp_mult_err_e(-lng.val, lng.err,
                                          sgn*F.val, F.err,
                                          result);
    return GSL_ERROR_SELECT_3(stat_e, stat_F, stat_g);
...
{
  EVAL_RESULT(Mathelp::mgsl_sf_hyperg_2F1_e(a, b, c, x, &result));
}

double mgsl_sf_hyperg_2F1_conj(double aR, double aI, double c, double x)
{
  EVAL_RESULT(Mathelp::mgsl_sf_hyperg_2F1_conj_e(aR, aI, c, x, &result));
}

double mgsl_sf_hyperg_2F1_renorm(double a, double b, double c, double x)
{
  EVAL_RESULT(Mathelp::mgsl_sf_hyperg_2F1_renorm_e(a, b, c, x, &result));
}

double mgsl_sf_hyperg_2F1_conj_renorm(double aR, double aI, double c, double x)
{
  EVAL_RESULT(Mathelp::mgsl_sf_hyperg_2F1_conj_renorm_e(aR, aI, c, x, &result));
}
